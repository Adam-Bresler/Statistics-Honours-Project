...
# Read in data from Lin and Juko
...
# Read 'match scores', a part of match statistics from a different csv
...
# Get rid of columns that are uninformative ----------------------------------
colnames(match)
rid <- c("match_stats_url_suffix", "match_time", "winner_slug.x", 
         "loser_slug.x", "tourney_order", "tourney_name", "tourney_slug.y",  
         "tourney_url_suffix", "start_year", "start_month", "start_day", 
         "end_year", "end_month", "end_day", "currency", "prize_money", 
         "winner_player_id", "winner_slug.y", "loser_player_id", 
         "loser_slug.y", "match_id.y")

match <- match[, -which(names(match) %in% rid)]


# Player names ---------------------------------------------------------------
# Gives a vector of unique player names
...
# Build player data base using webscraping -----------------------------------

# While this of course works, it is incredibly slow. Use the csv "players" instead, 
...
# Not necessary, but gives the foundation of code later
...
# Enables rolling average later by having correct tournament info
...
# To average, we make use of the order with in a players last games. 
# In script 2, this is vital. Thus we order as per tournament dates as well
# as matches within a tournament.
...
for(i in players_names){
  ...
  rows<- order(final_data[which(final_data$name==i),"tournament_date"],
               decreasing = FALSE)
  ...
  for(i in tourney_dates){
    ...
    rows2<- order(current_data2[which(current_data2$tournament_date==i),
                                "Match_order"],decreasing = TRUE)
...
# This was a rudimentary first attempt at historical averaging. Mathematically
# it is incorrect thus not used. Full averaging is done in script 2. 
